...
public class PlaylistBegin extends BaseCommand {
    public PlaylistBegin(ControlWrapper control_wrapper) {
        super(control_wrapper);
    }

    @Override
    public boolean call(KWargs options) {
        List<String> files = options.fetch("files", ArrayList.class, new ArrayList());
        if (files.size() > 20) {
